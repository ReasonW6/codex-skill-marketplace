...
在 Zen 的后台标签页里读取网页、点击、填写表单、导航和截图。通过 **Firefox 扩展 + Windows 原生通信宿主 + 本地 MCP** 接入 Codex。插件不调用系统鼠标键盘，不激活标签页，不聚焦窗口；用户切回受控标签页时，会释放控制。

这是独立开发的 Zen 适配插件。它提供经过实机验证的后台网页操作，**不等同于官方浏览器插件的全部能力**，也不会把 Zen 注册到官方 `@Browser` 入口。当前版本为 `0.1.0`，分发的浏览器扩展为 **未签名 XPI**，长期安装仍需要 Mozilla 签名。

## 能做什么

| 能力 | 行为 |
| --- | --- |
| 标签页 | 列出、创建后台标签、接管指定后台标签、释放控制 |
...
| 并行使用 | 每条 MCP 连接单独拥有标签；其他连接不能抢占；用户接管优先 |

现有容器标签页可以通过 `zen_attach` 使用。`zen_open` 不提供指定容器参数，也不索取读取全部 cookies 的权限。新建标签页默认静音；接管已有标签页不会改变其静音设置。
...
前台标签页、私密窗口和浏览器内部页面会被拒绝。用户切到受控标签页后，原来的控制权失效；切走也不会自动恢复。不要让模型为了继续工作而把用户切走。

扩展面板的“暂停控制”会断开本机宿主并释放全部标签。重新启用后，连接 ID 会变化，需要重新选取并接管目标。断线会保留网页；`zen_close` 只允许关闭当前连接新建且仍在后台的一个标签。
...
- `FOREGROUND_TAB` / `NOT_ATTACHED`：用户正在使用该标签，或已释放控制；不要反复自动接管。
